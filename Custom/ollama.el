...
(defcustom ollama:model "deepseek-r1"  ; Changed from "llama2-uncensored" to "deepseek-r1"
  "Ollama model."
  :group 'ollama
  :type 'string)
...
;;;###autoload
(defun ollama-ask-question ()
  "Ask a question directly through an input prompt."
  (interactive)
  (let ((question (read-string "Ask Ollama: ")))
    (with-output-to-temp-buffer "*ollama*"
      (princ
       (ollama-prompt ollama:endpoint question ollama:model)))))

;;;###autoload
(defun ollama-ask-about-region ()
  "Ask a question about the highlighted text or region."
  (interactive)
  (if (use-region-p)
      (let* ((region-text (buffer-substring (region-beginning) (region-end)))
             (question (read-string (format "Ask about the region: \"%s\": " region-text))))
        (with-output-to-temp-buffer "*ollama*"
          (princ
           (ollama-prompt ollama:endpoint (format "%s: %s" question region-text) ollama:model))))
